...
func (d DFTMatrixLiteral) GaloisElements(params Parameters) (galEls []uint64) {
	...
	for i, pVec := range indexCtS {
		GiantStep := he.OptimalLinearTransformationGiantStep(maps.Keys(pVec), dslots)
...
func (eval *DFTEvaluator) CoeffsToSlots(ctIn *rlwe.Ciphertext, ctsMatrices DFTMatrix, ctReal, ctImag *rlwe.Ciphertext) (err error) {
	...
	if ctsMatrices.Format == RepackImagAsReal || ctsMatrices.Format == SplitRealAndImag {
		...
		if err = eval.Evaluate(ctIn, ctsMatrices, zV); err != nil {
		...
		if err = eval.Evaluate(ctIn, ctsMatrices, ctReal); err != nil {
...
func (eval *DFTEvaluator) SlotsToCoeffs(ctReal, ctImag *rlwe.Ciphertext, stcMatrices DFTMatrix, opOut *rlwe.Ciphertext) (err error) {
	...
	if ctImag != nil {
		...
		if err = eval.Evaluate(opOut, stcMatrices, opOut); err != nil {
			return fmt.Errorf("eval.Evaluate: %w", err)
		}
	} else {
		if err = eval.Evaluate(ctReal, stcMatrices, opOut); err != nil {
			return fmt.Errorf("eval.Evaluate: %w", err)
		}
	}
...
func (eval *DFTEvaluator) Evaluate(ctIn *rlwe.Ciphertext, dft DFTMatrix, opOut *rlwe.Ciphertext) (err error) {
	...
	for _, Levels := range dft.Levels {
		for range Levels {
			if idx == 0 {
				if err = eval.LinearTransformationEvaluator.Evaluate(ctIn, dft.Matrices[idx], opOut); err != nil {
					return fmt.Errorf("eval.LinearTransformationEvaluator.Evaluate: %w", err)
				}
			} else {
				if err = eval.LinearTransformationEvaluator.Evaluate(opOut, dft.Matrices[idx], opOut); err != nil {
					return fmt.Errorf("eval.LinearTransformationEvaluator.Evaluate: %w", err)
				}
			}
