...
func (eval Evaluator) Evaluate(ctIn *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, err error) {
	...
	if eval.IterationsParameters == nil && eval.ResidualParameters.PrecisionMode() != hefloat.PREC128 {
		ctOut, _, err = eval.bootstrap(ctIn.Clone())
		return
	...
	if eval.EvkSparseToDense != nil {
		...
		for j := 0; j < N; j++ {
			...
			for i := 0; i < levelQ+1; i++ {
				tmp = ring.BRedAdd(coeff, Q[i], BRCQ[i])
				ks.BuffDecompQP[0][0].At(i)[j] = tmp*pos + (Q[i]-tmp)*neg

			}

			for i := 0; i < levelP+1; i++ {
				tmp = ring.BRedAdd(coeff, P[i], BRCP[i])
				ks.BuffDecompQP[0][1].At(i)[j] = tmp*pos + (P[i]-tmp)*neg
			}
		}

		for i := len(ks.BuffDecompQP) - 1; i >= 0; i-- {
			ringQ.NTT(ks.BuffDecompQP[0][0], ks.BuffDecompQP[i][0])
		}

		for i := len(ks.BuffDecompQP) - 1; i >= 0; i-- {
			ringP.NTT(ks.BuffDecompQP[0][1], ks.BuffDecompQP[i][1])
		}
		...
		ctTmp.Q = []ring.Poly{ks.BuffQ[1], ctIn.Q[1]}
