...
// And a failure probability of 2^{-133} for 2^{15} slots.
//
// =====================================
...
const (
	...
	// DefaultMod1Interval is the default interval [-K+1, K-1] for the polynomial approximation of the homomorphic modular reduction.
	DefaultMod1Interval = 16
...
// GetXs returns the Xs field of the target ParametersLiteral.
// The default value DefaultXs is returned if the field is nil.
func (p ParametersLiteral) GetXs() (Xs ring.DistributionParameters) {
	if v := p.Xs; v == nil {
		Xs = &DefaultXs
	} else {
		Xs = v
...
// GetXe returns the Xe field of the target ParametersLiteral.
// The default value DefaultXe is returned if the field is nil.
func (p ParametersLiteral) GetXe() (Xe ring.DistributionParameters) {
	if v := p.Xe; v == nil {
		Xe = &DefaultXe
	} else {
		Xe = v
...
func (p ParametersLiteral) GetLogP(NumberOfQi int) (LogP []int) {
	if v := p.LogP; v == nil {
		LogP = make([]int, max(1, int(math.Sqrt(float64(NumberOfQi)))))
		for i := range LogP {
			LogP[i] = 61
		}
	} else {
...
func (p ParametersLiteral) GetCoeffsToSlotsFactorizationDepthAndLogScales(LogSlots int) (CoeffsToSlotsFactorizationDepthAndLogScales [][]int, err error) {
	if p.CoeffsToSlotsFactorizationDepthAndLogScales == nil {
		CoeffsToSlotsFactorizationDepthAndLogScales = make([][]int, min(DefaultCoeffsToSlotsFactorizationDepth, max(LogSlots, 1)))
...
func (p ParametersLiteral) GetSlotsToCoeffsFactorizationDepthAndLogScales(LogSlots int) (SlotsToCoeffsFactorizationDepthAndLogScales [][]int, err error) {
	if p.SlotsToCoeffsFactorizationDepthAndLogScales == nil {
		SlotsToCoeffsFactorizationDepthAndLogScales = make([][]int, min(DefaultSlotsToCoeffsFactorizationDepth, max(LogSlots, 1)))
...
// GetMod1Interval returns the K field of the target ParametersLiteral.
// The default value DefaultK is returned if the field is nil.
func (p ParametersLiteral) GetMod1Interval() (Mod1Interval int, err error) {
	if v := p.Mod1Interval; v == nil {
		Mod1Interval = DefaultMod1Interval
	} else {
		Mod1Interval = *v

		if Mod1Interval < 0 {
			return Mod1Interval, fmt.Errorf("field Mod1Interval cannot be negative")
		}
	}
...
func (p ParametersLiteral) BinarySize() int {
...
func (p *ParametersLiteral) ReadFrom(r io.Reader) (n int64, err error) {
	...
	case buffer.Reader:
		...
		var inc int
		if inc, err = r.Read(data); err != nil {
			return n + int64(inc), fmt.Errorf("io.Reader.Read: %w", err)
		}

		return n + int64(inc), json.Unmarshal(data, p)
