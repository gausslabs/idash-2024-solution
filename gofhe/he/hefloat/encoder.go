...
// indicate that the expected input should be [][he.Float] or [][he.Complex].
...
// Encode encodes an [hefloat.FloatSlice] on the target plaintext.
...
func (ecd Encoder) Encode(values FloatSlice, pt *rlwe.Plaintext) (err error) {
	...
	if pt.IsBatched {
		...
		case []float64:
			...
			Float64ToFixedPointCRT(ecd.parameters.RingQ().AtLevel(pt.Level()), values, pt.Scale.Float64(), pt.Q.Coeffs)
		...
		case []big.Float:
			...
			BigFloatToFixedPointCRT(ecd.parameters.RingQ().AtLevel(pt.Level()), values, &pt.Scale.Value, pt.Q.Coeffs)
...
// Accepted polyOut.(type) are rlwe.Point and ring.Poly.
// The imaginary part will be discarded if ringType == ring.ConjugateInvariant.
...
func (ecd Encoder) embedDouble(values FloatSlice, metadata *rlwe.MetaData, polyOut interface{}) (err error) {
	...
	default:
		return fmt.Errorf("cannot Embed: invalid polyOut.(Type) must be rlwe.Point or ring.Poly but is %T", p)
	}
...
func (ecd Encoder) plaintextToFloat(level int, scale rlwe.Scale, logSlots int, p ring.Poly, values FloatSlice) (err error) {
	if level == 0 {
		return ecd.polyToFloatNoCRT(p.At(0), values, scale, logSlots, ecd.parameters.RingQ().AtLevel(level))
	}
...
func (ecd Encoder) decodePublic(pt *rlwe.Plaintext, values FloatSlice, logprec float64) (err error) {
	...
	if pt.IsNTT {
		ecd.parameters.RingQ().AtLevel(pt.Level()).INTT(pt.Q, ecd.buff)
	} else {
		ecd.buff.CopyLvl(pt.Level(), &pt.Q)
	}
	...
	case []complex128:
		...
		case []complex128:
			if true || logN < 4 {
				...
				SpecialiFFTDoubleUnrolled8(values, 1<<logN, ecd.m, ecd.rotGroup, roots)
			}
		default:
...
func polyToComplexCRT(poly ring.Poly, bigintCoeffs []big.Int, values FloatSlice, scale rlwe.Scale, logSlots int, isreal bool, r *ring.Ring) (err error) {

	maxCols := int(r.NthRoot() >> 2)
	slots := 1 << logSlots
	gap := maxCols / slots

	r.PolyToBigintCentered(poly, gap, bigintCoeffs)
	...
	case []complex128:
		...
		for i := 0; i < slots; i++ {
			values[i] = complex(scaleDown(&bigintCoeffs[i], scalef64), 0)
		}

		if !isreal {
			for i, j := 0, slots; i < slots; i, j = i+1, j+1 {
				values[i] += complex(0, scaleDown(&bigintCoeffs[j], scalef64))
			}
		} else {
			// [X]/(X^N+1) to [X+X^-1]/(X^N+1)
			slots := 1 << logSlots
	...
	case []bignum.Complex:
		...
		if !isreal {
			for i, j := 0, slots; i < slots; i, j = i+1, j+1 {
				values[i][1].SetInt(&bigintCoeffs[j])
			}
		} else {
			// [X]/(X^N+1) to [X+X^-1]/(X^N+1)
			slots := 1 << logSlots
...
func (ecd *Encoder) polyToFloatCRT(p ring.Poly, values FloatSlice, scale rlwe.Scale, logSlots int, r *ring.Ring) (err error) {
	...
	switch values := values.(type) {
	case []float64:
		slots = min(len(p.At(0)), len(values))
	case []complex128:
		slots = min(len(p.At(0)), len(values))
	case []big.Float:
		slots = min(len(p.At(0)), len(values))
	...
	r.PolyToBigintCentered(ecd.buff, 1, bigintCoeffs)
...
func (ecd *Encoder) polyToFloatNoCRT(coeffs []uint64, values FloatSlice, scale rlwe.Scale, logSlots int, r *ring.Ring) (err error) {
	...
	switch values := values.(type) {
	case []float64:
		slots = min(len(coeffs), len(values))
	case []complex128:
		slots = min(len(coeffs), len(values))
	case []big.Float:
		slots = min(len(coeffs), len(values))
