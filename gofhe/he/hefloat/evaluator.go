...
func (eval *Evaluator) addition(op0 *rlwe.Ciphertext, op1 rlwe.Operand, opOut *rlwe.Ciphertext, positive bool) (err error) {
	...
	case []complex128, []float64, []big.Float, []bignum.Complex:
		...
		pt, err := rlwe.NewPlaintextAtLevelFromPoly(level, -1, eval.BuffQ[0], ring.Poly{})
...
func (eval *Evaluator) evaluateInPlace(level int, c0, c1, opOut *rlwe.Ciphertext, evaluate func(ring.Poly, ring.Poly, ring.Poly)) {
	...
	if opOut.Vector == c0.Vector {
		...
		if cmp == 1 {
			...
			if ratioInt.Cmp(new(big.Int).SetUint64(0)) == 1 {
				...
				tmp1, err = rlwe.NewCiphertextAtLevelFromPoly(level, -1, eval.BuffCt.Q[:c1.Degree()+1], []ring.Poly{})
		...
		if cmp == 1 {
			...
			if ratioInt.Cmp(new(big.Int).SetUint64(0)) == 1 {
				...
				tmp0, err = rlwe.NewCiphertextAtLevelFromPoly(level, -1, eval.BuffCt.Q[:c0.Degree()+1], []ring.Poly{})
...
func (eval *Evaluator) RescaleTo(op0 *rlwe.Ciphertext, minScale rlwe.Scale, opOut *rlwe.Ciphertext) (err error) {
	...
	*opOut.MetaData = *op0.MetaData
	...
	for newLevel >= 0 {
		...
		scale = scale.Div(rlwe.NewScale(rQ.SubRings[newLevel].Modulus))
...
func (eval *Evaluator) divroundbylastmoduli(nbRescales int, op0, op1 *rlwe.Ciphertext) {
	...
	if nbRescales > 0 {
		...
		for i := range nbRescales {
			op1.Scale = op1.Scale.Div(rlwe.NewScale(rQ.SubRings[op0.Level()-i].Modulus))
		}
...
func (eval *Evaluator) Mul(op0 *rlwe.Ciphertext, op1 rlwe.Operand, opOut *rlwe.Ciphertext) (err error) {
	...
	case complex128, float64, int, int64, uint, uint64, *big.Int, *big.Float, *bignum.Complex, big.Int, big.Float, bignum.Complex:
		...
		if cmplxBig.IsInt() {
			...
			scale = rlwe.NewScale(rQ.SubRings[level].Modulus) // Current modulus scaling factor
			...
			for i := 1; i < eval.Parameters().LevelsConsumedPerRescaling(); i++ {
				scale = scale.Mul(rlwe.NewScale(rQ.SubRings[level-i].Modulus))
			}
		}
	...
	case []complex128, []float64, []big.Float, []bignum.Complex:
		...
		*pt.MetaData = *op0.MetaData
		pt.Scale = rlwe.NewScale(rQ.SubRings[level].Modulus)

		// If DefaultScalingFactor > 2^60, then multiple moduli are used per single rescale
		// thus continues multiplying the scale with the appropriate number of moduli
		for i := 1; i < eval.Parameters().LevelsConsumedPerRescaling(); i++ {
			pt.Scale = pt.Scale.Mul(rlwe.NewScale(rQ.SubRings[level-i].Modulus))
		}
...
func (eval *Evaluator) tensorStandardDegreeTwo(LevelQ int, op0, op1, op2 *rlwe.Ciphertext, c2 ring.Poly) {
	...
	case complex128, float64, int, int64, uint, uint64, *big.Int, *big.Float, *bignum.Complex, big.Int, big.Float, bignum.Complex:
		...
		if cmp := op0.Scale.Cmp(opOut.Scale); cmp == 0 {
			...
			if cmplxBig.IsInt() {
				...
				scaleRLWE = rlwe.NewScale(rQ.SubRings[level].Modulus)

				for i := 1; i < eval.Parameters().LevelsConsumedPerRescaling(); i++ {
					scaleRLWE = scaleRLWE.Mul(rlwe.NewScale(rQ.SubRings[level-i].Modulus))
				}
