...
func BRedAdd(a, q uint64, breconstant [2]uint64) (r uint64) {
	mhi, _ := bits.Mul64(a, breconstant[0])
	r = a - mhi*q
	if r >= q {
		r -= q
	}
...
func BRedAddLazy(x, q uint64, breconstant [2]uint64) uint64 {
	s0, _ := bits.Mul64(x, breconstant[0])
	return x - s0*q
}
...
func BRed(x, y, q uint64, breconstant [2]uint64) (r uint64) {
	...
	r = mhi * breconstant[0] // r = mhi * uhi

	hhi, hlo = bits.Mul64(mlo, breconstant[0]) // mlo * uhi

	r += hhi

	lhi, _ = bits.Mul64(mlo, breconstant[1]) // mlo * ulo

	s0, carry = bits.Add64(hlo, lhi, 0)

	r += carry

	hhi, hlo = bits.Mul64(mhi, breconstant[1]) // mhi * ulo
