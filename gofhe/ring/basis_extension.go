...
func ExtendBasisSmallNorm(Q uint64, P []uint64, pQ, pP Poly) {
...
func GenModUpConstants(Q, P []uint64) ModUpConstants {
	...
	for i := range Q {
		bredQ[i] = GetBRedConstant(Q[i])
		mredQ[i] = GetMRedConstant(Q[i])
	}

	for i := range P {
		bredP[i] = GetBRedConstant(P[i])
		mredP[i] = GetMRedConstant(P[i])
	}
	...
	for i, qi := range Q {
		...
		qoverqiinvqi[i] = ModexpMontgomery(qiStar, qi-2, qi, mredQ[i], bredQ[i])
...
func (be *BasisExtender) ModUpQtoP(levelQ, levelP int, polQ, polP Poly) {
	...
	QHalf := bignum.NewInt(ringQ.Modulus())
	QHalf.Rsh(QHalf, 1)
...
func (be *BasisExtender) ModUpPtoQ(levelP, levelQ int, polP, polQ Poly) {
	...
	PHalf := bignum.NewInt(ringP.Modulus())
	PHalf.Rsh(PHalf, 1)
...
func (be *BasisExtender) ModDownQPtoP(levelQ, levelP int, p1Q, p1P, p2P Poly) {
	...
	for i, s := range ringP.SubRings[:levelP+1] {
		// Then, for each coefficient we compute (P^-1) * (p1[i][j] - buff[i][j]) mod qi
		s.SubThenMulScalarMontgomeryTwoModulus(buffP.At(i), p1P.At(i), s.Modulus-modDownConstants[i], p2P.At(i))
	}
...
// Caution: values are not centered and returned values are in [0, 2P-1].
func ReconstructModP(p1 [][]uint64, p2 []uint64, ringQ *Ring, ringP *SubRing, MUC ModUpConstants) {
