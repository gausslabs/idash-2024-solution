...
func (r Ring) AutomorphismNTTWithIndex(polIn Poly, index []uint64, polOut Poly) {
	...
	for j := 0; j < N; j = j + 8 {
		...
		for i := 0; i < level+1; i++ {
			...
			z := (*[8]uint64)(unsafe.Pointer(&polOut.At(i)[j]))
			y := polIn.At(i)
...
func (r Ring) Automorphism(polIn Poly, gen uint64, polOut Poly) {
	...
	if r.Type() == ConjugateInvariant {
		...
		for i := uint64(0); i < 2*N; i++ {
			...
			if index < N {
				...
				for j, s := range r.SubRings[:level+1] {
					polOut.At(j)[index] = polIn.At(j)[idx]*(tmp^1) | (s.Modulus-polIn.At(j)[idx])*tmp
				}
			}
		}
		...
		for i := uint64(0); i < N; i++ {
			...
			for j, s := range r.SubRings[:level+1] {
				polOut.At(j)[index] = polIn.At(j)[i]*(tmp^1) | (s.Modulus-polIn.At(j)[i])*tmp
			}
		}
	}
}
